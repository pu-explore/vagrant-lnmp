...
# Path to the custom Lnmp $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.lnmp.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.lnmp.$(hostname).crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.lnmp.$(hostname).key"
...
then
    ...
    # Finally, generate the private key and certificate signed with the Lnmp $(hostname) Root CA.
